...
export class VCustomer extends VPage<CPost> {
    ...
    private page = observer(() => {
        ...
        let right = <div className="w-19c d-flex">
            <SearchBox className="w-80 mt-1 mr-2"
                size='sm'
                onSearch={(key: string) => this.controller.searchCustomerByKey(key, this.post.id)}
                placeholder="搜索客户姓名、单位" />
        </div>;
        let footer = <div className="d-block">
            <div className="w-100  justify-content-end" >
                <button type="button" className="btn btn-primary my-1 mr-3" style={{ padding: "6px 35px 6px 35px" }} onClick={this.share}>分享</button>
            </div>
        </div>;
        return <Page header="客户" onScrollBottom={this.onScrollBottom} headerClassName={setting.pageHeaderCss} right={right} footer={footer} >
    ...
    private renderCustomer = (customer: any, index: number) => {
        ...
        let date = <div className=" cursor-pointer small"><input type="checkbox" onChange={() => this.controller.cApp.postCustomer.add(id, this.post)} /></div>
        let left = webuser ?
    ...
    private share = async () => {
        ...
        let { caption, image, id, description } = post;
