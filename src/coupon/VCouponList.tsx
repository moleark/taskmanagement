...
export class VCouponList extends VPage<CCoupon> {
    ...
    private renderItem = (coupon: any, index: number) => {
        ...
        let bleft: any;
        if (couponCount > 0) {
            bleft = <small style={{ borderRadius: "15%/48%" }} className="bg-success small text-white px-3" >已使用 {couponCount} 次</small>;
        } else {
    ...
    private page = observer(() => {
        ...
        return <Page header={typename} onScrollBottom={this.onScrollBottom} headerClassName={setting.pageHeaderCss} right={right} >
